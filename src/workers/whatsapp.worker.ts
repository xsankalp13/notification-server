...
if (!QUEUE_URL) {
    console.error("❌ FATAL: WHATSAPP_QUEUE env var is not set.");
    process.exit(1);
}
 ...
 *
 * Error strategy:
 *  - TRANSIENT errors (e.g. WhatsApp API down): rethrow → do NOT delete message →
...
async function processMessage(msg: Message): Promise<void> {
    ...
    if (!event.parentPhone) {
        console.warn(
            `🗑️  Message for student "${event.studentName}" has no parentPhone – discarding.`
        );
        await deleteMessage(receiptHandle);
        return;
    }
    ...
    await deleteMessage(receiptHandle);
    console.log(`✅ WhatsApp processed and deleted for student: ${event.studentName}`);
}

async function poll() {
    console.log("🚀 WhatsApp worker started, polling:", QUEUE_URL);
